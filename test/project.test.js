...
describe("POST /", () => {
    it("3. Verifica que se puede crear un proyecto.", async () => {
        ...
        expect(res.body).to.have.all.keys('name', 'description', 'startDate', 'endDate', 'status', 'teamMembers', 'budget'); });//it
});//descri
...
describe('DELETE /:id', () => {
    it('5. Verificar que se puede eliminar un proyecto', async () => {
        ...
        expect(res.status).to.equal(200);
        expect(res.body).to.deep.equal({message: 'Projecto eliminado'});
    }); 
});
